...
struct AboutView: View {
    ...
    private var updateSection: some View {
        ...
        case .available(let version, _, _, let size):
            VStack(spacing: 8) {
                ...
                Text(formatSize(size))
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Button("Update Barktor") {
                    Task { await updater.updateBarktor() }
                }
                .buttonStyle(.borderedProminent)
                .disabled(!coordinator.safeToQuit)
                if coordinator.safeToQuit {
                    Text("Barktor will quit, replace itself, then relaunch.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                } else {
        ...
        case .readyToInstall:
            // Transient: updateBarktor() flips state -> .installing immediately

...
// Settings (or run `tccutil reset Accessibility com.naktor.barktor`
// and `tccutil reset ListenEvent com.naktor.barktor`) and grant
// again.
enum Permissions {
    enum Kind: String, CaseIterable, Identifiable {
        ...
        var why: String {
            switch self {
            case .microphone:
                return "Barktor needs access to record your voice."
            case .accessibility:
                return "Required so Barktor can paste your transcript at the cursor."
            case .inputMonitoring:
                return "Required so Barktor can listen for the global hotkey across every app."
            }
        }
        var settingsURL: URL {
    ...
    // Triggers the macOS prompt and adds Barktor to the Accessibility
